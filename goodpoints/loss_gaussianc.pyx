...
cdef double loss_gaussian_M_kernel_two_points(const double[:] X1,
    ...
    Base Gaussian kernel:
        k(X1, X2) = exp(-0.5 * ||X1-X2||_2^2/lam_sqd[j]) 
    Squared-loss Gaussian-M kernel:
        k_sq(X1, X2) = k(X1[:-product], X2[:-product])^2 
            + k(X1[:-product], X2[:-product]) * <X1[-product:], X2[-product:]>
    between two points X1 and X2
    ...
    Args:
      ...
      k_params: array of (sig_sqd, product), where
        ...
        product: number of dimensions to use for the product kernel 
            (counting backwards from the last dimension)
        ...: flattened feature matrix, where M_ij = k_params[2 + i*d_feat + j]
    """
    
    cdef double sig_sqd = k_params[0]
    cdef long product = <long>(k_params[1]) #1 if use product kernel, 0 otherwise
    cdef long d = X1.shape[0]
    cdef long d_feat = d - product
    
    cdef long i, j
    cdef double arg1, arg2, result
    
    # Compute the squared Mahalanobis distance between X1 and X2
    arg1 = 0
    for i in range(d_feat):
        for j in range(d_feat):
            # This computes (X1-X2)^T * M * (X1-X2) = \sum_{i,j} (X1[i]-X2[i]) * M_ij * (X1[j]-X2[j])
            # Note: M_ij = k_params[2 + i*d_feat + j] 
            # the 2 offset comes from the fact that
            # the first two elements of k_params are sig_sqd and product
            arg1 += (X1[i]-X2[i]) * k_params[2 + i*d_feat + j] * (X1[j]-X2[j])
...
'''
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Product Gaussian Kernel Functionality %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
'''
...
cdef double prod_gaussian_kernel_two_points(const double[:] X1,
    ...
    Nadaraya-Watson Gaussian kernel:
        k_sq(X1, X2) = k(X1[:-product], X2[:-product])
            + k(X1[:-product], X2[:-product]) * <X1[-product:], X2[-product:]>
    between two points X1 and X2
    ...
    # Compute the base Gaussian kernel
    result = exp(-0.5 * arg1 / sig_sqd)
